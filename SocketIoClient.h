...
#define DEFAULT_URL "/socket.io/?EIO=3"
...
private:
	...
	void trigger(const char* event, const char* payload, size_t length, const char* id = "");
...
public:
	...
	void loop();
	void on(const char* event, std::function<void (const char* payload, size_t length, const char* id)>);
	void emit(const char* event, const char* payload = NULL);
	void callback(const char* id, const char* payload = NULL);
	void disconnect();
};
